...
```bash
# Forward to service
kubectl port-forward -n maliev-dev svc/maliev-quotation-service 8080:8080
...
```bash
# Tail logs
kubectl logs -f deployment/maliev-quotation-service -n maliev-dev

# Get pod status
kubectl get pods -n maliev-dev | grep quotation-service
